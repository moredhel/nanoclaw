  ...
  private async connectInternal(onFirstOpen?: () => void): Promise<void> {
    ...
    this.sock.ev.on('connection.update', (update) => {
      ...
      if (connection === 'close') {
        ...
        // Use unavailable presence so WhatsApp doesn't suppress phone push notifications.
        // Typing indicators still work — we send per-chat presence updates when composing.
    ...
    this.sock.ev.on('messages.upsert', async ({ messages }) => {
      for (const msg of messages) {
        ...
        // Extract basic text content (available for all chats)
        let content =
        ...
        // Heavy processing (image download, voice transcription) only for registered groups
        if (isRegistered) {
        ...
        // Registered-group voice messages are handled above and always produce non-empty content.
        if (!content) continue;
